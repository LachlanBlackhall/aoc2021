...
def flash(flashed, step_flashes, flash_count, queue, x, y, car):
    flashed.append(f'{x};{y}')
    flash_count += 1
    step_flashes += 1
    queue.append(f'{x};{y}')
    # print(f'Added {x};{y} to queue from direction {car}')
    return flashed, step_flashes, flash_count, queue
    ...
    for y, line in enumerate(split_list):
        for x, num in enumerate(line):
            ...
            if int(copy_list[y][x]) >= 9 and f'{x};{y}' not in flashed: 
                flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, x, y, 'O')
                while queue:
                    try:
                        ...
                        if int(copy_list[s_y-1][s_x-1]) >= 9 and f'{s_x-1};{s_y-1}' not in flashed and int(copy_list[s_y-1][s_x-1]) != 99999:
                            flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, s_x-1, s_y-1, 'NW')
                        ...
                        if int(copy_list[s_y-1][s_x]) >= 9 and f'{s_x};{s_y-1}' not in flashed and int(copy_list[s_y-1][s_x]) != 99999:
                            flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, s_x, s_y-1, 'N')
                        elif int(copy_list[s_y-1][s_x]) != 99999:
                            copy_list[s_y-1][s_x] += 1
                        if int(copy_list[s_y-1][s_x+1]) >= 9 and f'{s_x+1};{s_y-1}' not in flashed and int(copy_list[s_y-1][s_x+1]) != 99999:
                            flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, s_x+1, s_y-1, 'NE')
                        elif int(copy_list[s_y-1][s_x+1]) != 99999:
                            copy_list[s_y-1][s_x+1] += 1
                        if int(copy_list[s_y][s_x-1]) >= 9 and f'{s_x-1};{s_y}' not in flashed and int(copy_list[s_y][s_x-1]) != 99999:
                            flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, s_x-1, s_y, 'E')
                        elif int(copy_list[s_y][s_x-1]) != 99999:
                            copy_list[s_y][s_x-1] += 1
                        if int(copy_list[s_y][s_x+1]) >= 9 and f'{s_x+1};{s_y}' not in flashed and int(copy_list[s_y][s_x+1]) != 99999:
                            flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, s_x+1, s_y, 'W')
                        elif int(copy_list[s_y][s_x+1]) != 99999:
                            copy_list[s_y][s_x+1] += 1
                        if int(copy_list[s_y+1][s_x-1]) >= 9 and f'{s_x-1};{s_y+1}' not in flashed and int(copy_list[s_y+1][s_x-1]) != 99999:
                            flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, s_x-1, s_y+1, 'SW')
                        elif int(copy_list[s_y+1][s_x-1]) != 99999:
                            copy_list[s_y+1][s_x-1] += 1
                        if int(copy_list[s_y+1][s_x]) >= 9 and f'{s_x};{s_y+1}' not in flashed and int(copy_list[s_y+1][s_x]) != 99999:
                            flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, s_x, s_y+1, 'S')
                        elif int(copy_list[s_y+1][s_x]) != 99999:
                            copy_list[s_y+1][s_x] += 1
                        if int(copy_list[s_y+1][s_x+1]) >= 9 and f'{s_x+1};{s_y+1}' not in flashed and int(copy_list[s_y+1][s_x+1]) != 99999:
                            flashed, step_flashes, flash_count, queue = flash(flashed, step_flashes, flash_count, queue, s_x+1, s_y+1, 'SE')
                        elif int(copy_list[s_y+1][s_x+1]) != 99999:
                            copy_list[s_y+1][s_x+1] += 1                 
