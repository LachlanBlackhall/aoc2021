...
def check_cardinals(num_char, north, east, south, west):
    if int(num_char) >= int(north):
        return False
    if int(num_char) >= int(east):
        return False
    if int(num_char) >= int(south):
        return False
    if int(num_char) >= int(west):
        return False
    return True
...
def check_basin(coords, split_list):
    ...
    while queue:
        ...
        size += 1
        num_char = split_list[y][x]
        north = split_list[y-1][x]
        east = split_list[y][x+1]
        south = split_list[y+1][x]
        west = split_list[y][x-1]
        if (int(num_char) < int(north) and north != '9'
                and f'{x};{y-1}' not in visited and [x, y-1] not in queue):
            queue.append([x, y-1])
        if (int(num_char) < int(east) and east != '9'
                and f'{x+1};{y}' not in visited and [x+1, y] not in queue):
            queue.append([x+1, y])
        if (int(num_char) < int(south) and south != '9'
                and f'{x};{y+1}' not in visited and [x, y+1] not in queue):
            queue.append([x, y+1])
        if (int(num_char) < int(west) and west != '9'
                and f'{x-1};{y}' not in visited and [x-1, y] not in queue):
            queue.append([x-1, y])
    return size


# Start by adding a wall of 9's around the input
...
for y, line in enumerate(split_list):
    if y in (0, len(split_list)-1):
        continue
    for x, num_char in enumerate(line):
        if x == 0 or x == (len(line)-1) or num_char == '9':
            continue
        # Check the four cardinal directions
        if check_cardinals(num_char, split_list[y-1][x], split_list[y][x+1],
